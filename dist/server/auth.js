...
const changePasswordSchema = z
    ...
    .refine((value) => String(value.newPassword) === String(value.confirmPassword), {
...
async function getAccountDataRow(userId) {
...
function normalizeMarketingPreferences(value = {}) {
    const source = value && typeof value === "object" ? value : {};
    return {
        email: Boolean(source.email),
        phone: Boolean(source.phone),
...
function normalizePrivateCarePreferences(value = {}) {
    const source = value && typeof value === "object" ? value : {};
    return {
        email: Boolean(source.email),
        phone: Boolean(source.phone),
...
function normalizePrivateCareHistory(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .map((entry) => {
...
function normalizeRepairsHistory(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .map((entry) => {
...
async (req, res) => {
    ...
    try {
        if (!user.emailVerified) {
            ...
            if (!accountCode.ok)
                return res.status(500).json({ error: "AUTH_CODE_ISSUE_FAILED" });
            return res.status(200).json(buildEmailCodeResponseBase(user.email, "account_verification_required", accountCode.issued));
        }
        if (user.passwordResetRequired) {
            const resetCode = await issueAndSendPasswordResetCode(user);
            if (!resetCode.ok)
...
authRouter.post("/forgot-password", forgotPasswordLimiter, async (req, res) => {
    ...
    try {
        ...
        const configuredProvider = String(process.env.EMAIL_PROVIDER || (String(process.env.RESEND_API_KEY || "").trim() ? "resend" : "console"))
            .trim()
            .toLowerCase();
...
myRouter.post("/orders/:orderId/refund", requireAuth, async (req, res) => {
    ...
    if (!isRefundWindowOpen(order))
        return res.status(409).json({ error: "REFUND_WINDOW_EXPIRED" });
    const stripe = getStripeClient();
    if (!stripe)
        return res.status(500).json({ error: "STRIPE_NOT_CONFIGURED" });
    try {
...
myRouter.put("/password", requireAuth, async (req, res) => {
...
myRouter.post("/addresses", requireAuth, async (req, res) => {
    ...
    const nextList = normalizeAddressList([...current.addresses, address], address.isDefault ? address.id : current.defaultAddressId);
...
myRouter.put("/addresses/:addressId", requireAuth, async (req, res) => {
    ...
    const addresses = current.addresses.map((address) => address.id === addressId
        ? {
            ...address,
            ...parsed.data,
            updatedAt: new Date().toISOString()
...
myRouter.get("/favorites", requireAuth, async (req, res) => {
...
myRouter.put("/preferences", requireAuth, async (req, res) => {
...
myRouter.get("/private-care", requireAuth, async (req, res) => {
    const row = await getAccountDataRow(req.session.userId);
    if (!row)
        return res.status(404).json({ error: "USER_NOT_FOUND" });
    return res.json({
...
myRouter.put("/private-care/preferences", requireAuth, async (req, res) => {
...
myRouter.post("/private-care", requireAuth, async (req, res) => {
...
myRouter.post("/avatar", requireAuth, avatarUpload.single("avatar"), async (req, res) => {
    if (!req.file)
