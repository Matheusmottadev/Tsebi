...
const { listShipmentsByOrderIds, upsertShipmentPending, updateShipmentTracking } = require("../src/db/queries/shipping.queries");
const { INTERNAL_TRACKING_STATES, mapMelhorEnvioStatusToInternal } = require("../src/shipping/melhorenvio-status");
...
async function recordAuditLog(req, payload) {
    ...
    const changedFields = Array.isArray(payload.changedFields) && payload.changedFields.length > 0
...
async function applyAuditReverseOperation(log) {
    ...
    if (type === "user_delete") {
        ...
        if (!targetId)
            throw createAdminError("AUDIT_INVALID_PAYLOAD", 400);
    ...
    if (type === "user_restore") {
        ...
        if (restored?.error === "INVALID_SNAPSHOT")
            throw createAdminError("AUDIT_INVALID_PAYLOAD", 400);
        if (restored?.error === "EMAIL_ALREADY_EXISTS")
            throw createAdminError("AUDIT_RESTORE_EMAIL_CONFLICT", 409);
    ...
    if (type === "product_delete") {
        ...
        if (before) {
            ...
            if (removed?.error === "PRODUCT_IN_USE")
                throw createAdminError("PRODUCT_IN_USE", 409);
        }
    ...
    if (type === "product_restore") {
        ...
        if (restored?.error === "INVALID_SNAPSHOT")
            throw createAdminError("AUDIT_INVALID_PAYLOAD", 400);
        if (restored?.error === "SKU_ALREADY_EXISTS")
            throw createAdminError("SKU_ALREADY_EXISTS", 409);
...
adminRouter.get("/orders", async (req, res) => {
    ...
    try {
        ...
        const userIds = Array.from(new Set(paged.rows
        ...
        const shipmentsByOrderId = await listShipmentsByOrderIds(paged.rows.map((order) => String(order.id)).filter(Boolean));
...
adminRouter.patch("/orders/:id", async (req, res) => {
    ...
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(parsed.data, "trackingCode")) {
            ...
            if (trackingCode) {
                const provider = String(afterState.shippingSelectedProvider || afterState.shipping?.shippingProvider || "manual")
                    .trim()
                    .toLowerCase() || "manual";
                const serviceCode = String(afterState.shippingSelectedServiceCode || afterState.shipping?.shippingServiceCode || "manual")
                    .trim() || "manual";
...
adminRouter.post("/products/:id/image", sensitiveAdminRateLimit, express.raw({
