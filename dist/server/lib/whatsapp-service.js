...
const { normalizePhone, findContactByPhone, listVipContactsRaw, insertSendLog } = require("./whatsapp-repository");
...
async function sendOrderShippedWhatsApp(order, shipment) {
    ...
    if (!phone)
        return { ok: false, skipped: "NO_PHONE" };
    const trackingCode = String(shipment?.trackingCode || shipment?.tracking_code || order?.trackingCode || order?.tracking_code || "").trim();
    const freeAllowed = await canSendFreeMessage(phone);
...
async function sendNewCollectionToVIP(collectionName, message) {
    ...
    for (let i = 0; i < vipContacts.length; i += batchSize) {
        ...
        for (const contact of chunk) {
            ...
            const error = result.ok ? null : String(result.error || "UNKNOWN_ERROR");
