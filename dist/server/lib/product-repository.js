...
function sanitizeVariantStockMap(value, validColors = [], validSizes = []) {
    ...
    Object.entries(value).forEach(([rawKey, rawQty]) => {
        ...
        const canonicalKey = splitByPipe.length === 2
            ? `${String(splitByPipe[0] || "").trim()}__${String(splitByPipe[1] || "").trim()}`
            : key;
        if (!canonicalKey || !allowedPairs.has(canonicalKey))
            return;
...
async function queryWithOptionalMetadata(sqlWithMetadata, sqlWithoutMetadata, params = [], fallbackParams = null) {
    try {
        ...
        if (!isMissingMetadataColumnError(error) || !sqlWithoutMetadata)
            throw error;
        ...
        try {
            ...
            if (!isMissingMetadataColumnError(retryError))
                throw retryError;
...
async function searchAdminProducts({ query: q = "", status = "", stock = "", page = 1, pageSize = 50 } = {}) {
    const safePage = Math.max(1, Number(page) || 1);
...
function sumVariantStock(metadata) {
    const map = metadata?.variantStock && typeof metadata.variantStock === "object" && !Array.isArray(metadata.variantStock)
...
async function createProduct(payload = {}) {
    ...
    try {
      ...
      `, paramsWithMetadata, paramsWithoutMetadata);
...
function getSnapshotString(snapshot, key) {
    return String(snapshot[key] || "");
}
async function restoreProductFromSnapshot(snapshotInput = {}) {
