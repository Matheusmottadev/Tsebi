...
function resolveActorKey(userId, anonId) {
...
function priceBucketFromCents(value) {
...
async function sendToPosthogServer(input, actorKey, eventId) {
    if (!posthogClient)
        return;
...
async function sendMetaConversionsEvent(input, eventId) {
    const normalizedName = normalizeText(input.eventName);
    const mapped = META_EVENT_MAP[normalizedName];
    if (!mapped || !metaEnabled || !metaPixelId || !metaCapiToken)
        return;
    const payload = {
        data: [
            {
    ...
    await fetch(`https://graph.facebook.com/${encodeURIComponent(metaApiVersion)}/${encodeURIComponent(metaPixelId)}/events?access_token=${encodeURIComponent(metaCapiToken)}`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(payload),
    }).catch(() => { });
}
async function upsertAffinity(client, actorKey, userId, anonId, affinityKey, delta) {
    if (!affinityKey || !Number.isFinite(delta) || delta === 0)
        return;
...
async function upsertProfileOnEvent(client, actorKey, userId, anonId, eventName, category, priceCents) {
    const existing = await client.query(`SELECT top_categories, ltv_cents, purchase_count, avg_ticket_cents FROM recommendation_profiles WHERE actor_key = $1 LIMIT 1`, [actorKey]);
    const row = existing.rows[0] || {};
    const counters = row.top_categories && typeof row.top_categories === "object" ? { ...row.top_categories } : {};
    if (category) {
...
async function logBehaviorEvent(input) {
    ...
    const outboundTasks = [sendToPosthogServer(input, actorKey, eventId)];
...
async function mergeAnonymousIdentity(input) {
    const safeAnonId = normalizeText(input.anonId);
    const safeUserId = normalizeText(input.userId);
    if (!safeAnonId || !safeUserId)
        return { ok: false, reason: "INVALID_INPUT" };
    const anonActor = resolveActorKey("", safeAnonId);
    const userActor = resolveActorKey(safeUserId, "");
    await withTransaction(async (client) => {
        await client.query(`UPDATE behavior_events SET actor_key = $1, user_id = $2, anon_id = NULL WHERE actor_key = $3`, [userActor, safeUserId, anonActor]);
        await client.query(`
      INSERT INTO user_affinity (actor_key, user_id, anon_id, affinity_key, score, updated_at)
      ...
      `, [userActor, safeUserId, anonActor]);
        ...
        const profileRows = await client.query(`SELECT actor_key, ltv_cents, purchase_count, avg_ticket_cents, top_categories, favorite_price_bucket FROM recommendation_profiles WHERE actor_key IN ($1, $2)`, [userActor, anonActor]);
        let mergedLtv = 0;
        ...
        for (const row of profileRows.rows) {
            ...
            if (!mergedBucket)
                mergedBucket = normalizeText(row.favorite_price_bucket);
            const counters = row.top_categories && typeof row.top_categories === "object" ? row.top_categories : {};
            Object.entries(counters).forEach(([key, value]) => {
                categories[key] = Math.max(0, Number(categories[key] || 0) + Number(value || 0));
            });
        }
        if (mergedPurchaseCount > 0)
            mergedAvg = Math.floor(mergedLtv / mergedPurchaseCount);
        await client.query(`
      INSERT INTO recommendation_profiles (
        actor_key, user_id, anon_id, ltv_cents, purchase_count, avg_ticket_cents, top_categories, favorite_price_bucket, last_seen_at, updated_at
...
function resolveProductPriceBandFromValue(priceValue, thresholds) {
    if (!Number.isFinite(priceValue) || priceValue <= thresholds.lowMax)
        return "<500";
    if (priceValue <= thresholds.midMax)
        return "500-1000";
    return "1000+";
}
function resolveBandThresholds(products) {
    const prices = products
        .map((item) => Math.max(0, Number(item?.priceValue || 0)))
        .filter((value) => value > 0)
        .sort((a, b) => a - b);
    if (!prices.length)
        return { lowMax: 500, midMax: 1000 };
...
async function getRecommendationsForActor(input) {
    ...
    const ranked = activeProducts.map((product) => {
        ...
        if (favoriteBucket && favoriteBucket === bucket)
            score += 2.5;
        if (material)
            score += Number(affinityMap.get(`attr:material:${material}`) || 0) * 1.8;
        ...
        if (topProduct) {
            if (sanitizeToken(topProduct.category) === cat)
                score += 4;
    ...
    const selected = personalized.length > 0
        ? personalized
        : activeProducts
            .slice()
            .sort((a, b) => {
...
//# sourceMappingURL=behavior-analytics-repository.js.map