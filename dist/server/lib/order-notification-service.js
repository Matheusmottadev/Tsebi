...
const { sendOrderConfirmedEmail, sendPaymentApprovedEmail, sendOrderShippedEmail, sendOrderOutForDeliveryEmail, sendOrderDeliveredEmail } = require("./email-service");
...
async function safeNotify(kind, handler, context = {}) {
    try {
        ...
        return { ok: false, kind, error: String(error?.message || "ORDER_EMAIL_FAILED") };
    }
}
async function notifyOrderConfirmed(order) {
    const to = getOrderRecipientEmail(order);
    if (!to)
        return { ok: false, skipped: "NO_EMAIL_RECIPIENT" };
    return safeNotify("order_confirmed", async () => {
...
async function notifyPaymentApproved(order) {
    ...
    if (!to)
        return { ok: false, skipped: "NO_EMAIL_RECIPIENT" };
    return safeNotify("payment_approved", async () => {
...
async function notifyOrderShipped(order, shipment) {
    ...
    if (!to)
        return { ok: false, skipped: "NO_EMAIL_RECIPIENT" };
    return safeNotify("order_shipped", async () => {
...
async function notifyShipmentMilestoneTransition({ order, previousStatus, nextStatus, shipment }) {
    ...
    if (nextMilestone === "out_for_delivery") {
        return safeNotify("order_out_for_delivery", async () => {
            await sendOrderOutForDeliveryEmail({ to, order, shipment });
        }, { orderId: order?.id });
    }
    if (nextMilestone === "delivered") {
        return safeNotify("order_delivered", async () => {
