...
let repairSchemaPromise = null;
function createRepairError(code, status = 400, message = code) {
...
function normalizeRepairStatus(value) {
    ...
    if (["in_repair", "em_reparo", "em reparo"].includes(raw))
        return "in_repair";
    if (["completed", "finalizado", "concluido", "concluído"].includes(raw))
        return "completed";
    if (["returned", "devolvido"].includes(raw))
        return "returned";
    if (["rejected", "rejeitado", "recusado"].includes(raw))
        return "rejected";
    return "pending";
}
function normalizeRepairDecisionOutcome(value) {
    const raw = normalizeText(value).toLowerCase();
    if (raw === "accepted")
        return "accepted";
    if (raw === "rejected")
...
function normalizePhotoRow(entry) {
...
async function createRepairRequest(input) {
    ...
    if (!row)
        throw createRepairError("REPAIR_CREATE_FAILED", 500, "Falha ao criar solicitacao de reparo.");
    return mapRepairRow(row);
}
async function listMyRepairRequests(userId) {
    await ensureRepairTables();
...
async function getRepairRequestById(repairId) {
    await ensureRepairTables();
...
async function updateRepairRequestStatus(repairId, input) {
    await ensureRepairTables();
    const safeRepairId = normalizeText(repairId);
    if (!safeRepairId)
        throw createRepairError("INVALID_ID", 400, "Solicitacao invalida.");
    const action = input.action === "decision" ? "decision" : "progress";
    ...
    const pieceReceivedAt = Object.prototype.hasOwnProperty.call(input, "pieceReceivedAt") ? normalizeOptionalIsoDate(input.pieceReceivedAt) : undefined;
    const returnPostedAt = Object.prototype.hasOwnProperty.call(input, "returnPostedAt") ? normalizeOptionalIsoDate(input.returnPostedAt) : undefined;
    const returnedDeliveredAt = Object.prototype.hasOwnProperty.call(input, "returnedDeliveredAt")
    ...
    return withTransaction(async (client) => {
      ...
      `, [safeRepairId]);
        ...
        if (!current)
            throw createRepairError("REPAIR_NOT_FOUND", 404, "Solicitacao nao encontrada.");
        ...
        const nextDecisionOutcome = action === "decision" ? (nextStatus === "rejected" ? "rejected" : "accepted") : currentMapped.decisionOutcome;
        const nextDecisionReason = action === "decision"
            ? nextStatus === "rejected"
                ? rejectionReason
                : adminNote
        ...
        const nextTrackingCode = Object.prototype.hasOwnProperty.call(input, "trackingCode") ? trackingCode : currentMapped.trackingCode;
        ...
        const nextReturnedDeliveredAt = returnedDeliveredAt === undefined ? currentMapped.returnedDeliveredAt : returnedDeliveredAt;
      ...
      `, [
        ...
        if (!row)
            throw createRepairError("REPAIR_REVIEW_FAILED", 500, "Falha ao atualizar solicitacao.");
        return {
...
//# sourceMappingURL=repairs-repository.js.map