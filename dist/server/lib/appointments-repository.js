...
let appointmentSchemaPromise = null;
function createAppointmentError(code, status = 400, message = code) {
...
function normalizeAppointmentStatus(value) {
    ...
    if (raw === "completed" || raw === "concluido" || raw === "concluído")
        return "completed";
    if (raw === "canceled" || raw === "cancelado" || raw === "cancelled")
        return "canceled";
    return "scheduled";
}
function normalizeSlotStatus(value) {
    const raw = normalizeText(value).toLowerCase();
    if (["available", "disponivel", "disponível"].includes(raw))
        return "available";
    if (["unavailable", "indisponivel", "indisponível"].includes(raw))
        return "unavailable";
    if (["blocked", "bloqueado"].includes(raw))
        return "blocked";
    if (["filled", "lotado"].includes(raw))
        return "filled";
    if (["booked", "agendado"].includes(raw))
        return "booked";
    return "";
}
...
function mapSlotRow(row, appointments = []) {
...
async function listAppointmentsBySlotIds(slotIds) {
    if (!slotIds.length)
        return new Map();
...
async function listAppointmentSlotsForDate(date) {
    await ensureAppointmentTables();
    const safeDate = normalizeDateKey(date);
    if (!safeDate)
        throw createAppointmentError("INVALID_DATE", 400, "Data invalida.");
...
async function createAppointment(input) {
    ...
    return withTransaction(async (client) => {
      ...
      `, [slotId]);
        ...
        if (!slotRow)
            throw createAppointmentError("SLOT_NOT_FOUND", 404, "Horario nao encontrado.");
      ...
      `, [slotId, userId, serviceType, modality || slot.modality || "", notes]);
        ...
        if (!appointmentRow)
            throw createAppointmentError("APPOINTMENT_CREATE_FAILED", 500, "Falha ao criar agendamento.");
...
async function createAdminAppointmentSlot(input) {
    ...
    if (!startsAt || !endsAt)
        throw createAppointmentError("INVALID_INPUT", 400, "Horario invalido.");
    ...
    if (!row)
        throw createAppointmentError("SLOT_CREATE_FAILED", 500, "Falha ao criar horario.");
    return mapSlotRow(row, []);
}
async function updateAdminAppointmentSlot(slotId, patch) {
    await ensureAppointmentTables();
    const safeSlotId = normalizeText(slotId);
    if (!safeSlotId)
        throw createAppointmentError("INVALID_ID", 400, "Horario invalido.");
    return withTransaction(async (client) => {
      ...
      `, [
        ...
        if (!row)
            throw createAppointmentError("SLOT_UPDATE_FAILED", 500, "Falha ao atualizar horario.");
...
async function deleteAdminAppointmentSlot(slotId) {
    await ensureAppointmentTables();
    const safeSlotId = normalizeText(slotId);
    if (!safeSlotId)
        throw createAppointmentError("INVALID_ID", 400, "Horario invalido.");
...
async function cancelAdminAppointment(appointmentId) {
    ...
    if (!safeId)
        throw createAppointmentError("INVALID_ID", 400, "ID invalido.");
    return withTransaction(async (client) => {
        const checkResult = await client.query(`SELECT id, status FROM appointments WHERE id = $1::uuid LIMIT 1 FOR UPDATE`, [safeId]);
        const existing = checkResult.rows[0];
        if (!existing)
            throw createAppointmentError("APPOINTMENT_NOT_FOUND", 404, "Agendamento nao encontrado.");
        ...
        await client.query(`UPDATE appointments SET status = 'canceled', updated_at = NOW() WHERE id = $1::uuid`, [safeId]);
        const joined = await client.query(`SELECT a.id, a.slot_id, a.user_id, a.status, a.service_type, a.modality, a.notes, a.admin_note, a.created_at, a.updated_at,
              u.name AS user_name, u.email AS user_email,
              s.starts_at AS slot_starts_at, s.ends_at AS slot_ends_at, s.label AS slot_label, s.location AS slot_location
       FROM appointments a
       JOIN users u ON u.id = a.user_id
       JOIN appointment_slots s ON s.id = a.slot_id
       WHERE a.id = $1::uuid LIMIT 1`, [safeId]);
...
async function rescheduleAdminAppointment(appointmentId, newSlotId) {
    await ensureAppointmentTables();
    const safeId = normalizeText(appointmentId);
    const safeNewSlotId = normalizeText(newSlotId);
    if (!safeId || !safeNewSlotId)
        throw createAppointmentError("INVALID_INPUT", 400, "Dados invalidos.");
    return withTransaction(async (client) => {
        const apptResult = await client.query(`SELECT a.id, a.slot_id, a.user_id, a.status, a.service_type, a.modality, a.notes, a.admin_note, a.created_at, a.updated_at,
              u.name AS user_name, u.email AS user_email,
              s.starts_at AS slot_starts_at, s.ends_at AS slot_ends_at, s.label AS slot_label, s.location AS slot_location
       FROM appointments a
       JOIN users u ON u.id = a.user_id
       JOIN appointment_slots s ON s.id = a.slot_id
       WHERE a.id = $1::uuid LIMIT 1 FOR UPDATE`, [safeId]);
        ...
        const newSlotResult = await client.query(`SELECT * FROM appointment_slots WHERE id = $1::uuid LIMIT 1 FOR UPDATE`, [safeNewSlotId]);
        const newSlotRow = newSlotResult.rows[0];
        if (!newSlotRow)
            throw createAppointmentError("SLOT_NOT_FOUND", 404, "Novo horario nao encontrado.");
        ...
        const countResult = await client.query(`SELECT COUNT(*)::text AS booked_count FROM appointments
       WHERE slot_id = $1::uuid AND status <> 'canceled' AND id <> $2::uuid`, [safeNewSlotId, safeId]);
        const bookedCount = Math.max(0, Number(countResult.rows[0]?.booked_count || 0));
        if (bookedCount >= newSlot.capacity) {
            throw createAppointmentError("SLOT_FULL", 409, "Novo horario sem vagas.");
        }
        await client.query(`UPDATE appointments SET slot_id = $2::uuid, status = 'scheduled', updated_at = NOW() WHERE id = $1::uuid`, [safeId, safeNewSlotId]);
        const updatedResult = await client.query(`SELECT a.id, a.slot_id, a.user_id, a.status, a.service_type, a.modality, a.notes, a.admin_note, a.created_at, a.updated_at,
              u.name AS user_name, u.email AS user_email,
              s.starts_at AS slot_starts_at, s.ends_at AS slot_ends_at, s.label AS slot_label, s.location AS slot_location
       FROM appointments a
       JOIN users u ON u.id = a.user_id
