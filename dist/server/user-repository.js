...
async function findUserByEmail(email) {
    ...
    if (!normalizedEmail)
        return null;
    const result = await query(`SELECT * FROM users WHERE lower(email) = lower($1) LIMIT 1`, [normalizedEmail]);
...
async function listUsers({ limit = 100, offset = 0, search = "" } = {}) {
    ...
    LIMIT $1 OFFSET $2
    `, values);
    return result.rows.map(fromRow).filter((item) => Boolean(item));
}
 ...
 */
async function searchUsersAdmin({ query: q = "", status = "", page = 1, pageSize = 50 } = {}) {
...
async function updateUser(id, patch) {
    ...
    const next = {
        ...
        passwordResetRequired: patch.passwordResetRequired == null
            ? patch.passwordHash
                ? false
 ...
 * @returns {Promise<Record<string, unknown> | null>}
 */
async function setGuestTempPasswordIfMissing(id, passwordHash) {
    await ensureUserSecurityColumns();
...
async function adminRestoreUserAuthSnapshot(id, snapshot = {}) {
    ...
    const loginDisabled = snapshot.loginDisabled == null ? Boolean(current.loginDisabled) : Boolean(snapshot.loginDisabled);
    const passwordResetRequired = snapshot.passwordResetRequired == null
        ? Boolean(current.passwordResetRequired)
...
async function restoreUserFromSnapshot(snapshot = {}) {
    ...
    try {
      ...
      `, [
            ...
            JSON.stringify(Array.isArray(snapshot.adminMfaRecoveryCodes)
 ...
 * @returns {Promise<{ok: true; user: Record<string, unknown> | null} | {ok: false; error: string}>}
 */
async function upsertCheckoutGuestUser({ name = "", email = "", phone = "", cpf = "", cep = "", shippingAddress = null } = {}) {
    await ensureUserSecurityColumns();
    const normalizedEmail = normalizeEmail(email);
    if (!normalizedEmail) {
