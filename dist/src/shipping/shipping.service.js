...
const { normalizeZip, saveShippingQuotes, findShippingQuoteById, assignShippingQuoteToOrder, applyShippingSelectionToOrder, upsertShipmentPending, findShipmentByOrderId, markShipmentLabelPurchased, updateShipmentTracking } = require("../db/queries/shipping.queries");
const { updateOrderTrackingState, insertTrackingEvents } = require("../db/queries/order-tracking.queries");
...
function getConfiguredProviderName() {
    ...
    if (explicit)
        return explicit;
    if (String(process.env.MELHOR_ENVIO_TOKEN || "").trim())
        return "melhorenvio";
    if (String(process.env.MELHORENVIO_CLIENT_ID || "").trim())
        return "melhorenvio";
    if (String(process.env.MELHORENVIO_BASE_URL || "").trim())
        return "melhorenvio";
    return "dummy";
}
function getProvider(providerName = "") {
    const normalized = String(providerName || getConfiguredProviderName()).trim().toLowerCase();
    if (normalized === "melhorenvio")
        return assertShippingProvider(melhorEnvioProvider, "melhorenvio");
    if (normalized === "dummy")
        return assertShippingProvider(dummyProvider, "dummy");
...
async function buyLabelForOrder(order) {
    ...
    const carrierName = String(order?.carrier || order?.shippingSelectedCarrierName || "Melhor Envio").trim() || "Melhor Envio";
...
async function getLabelForOrder(order) {
    ...
    if (!shipment)
        throw createShippingError("SHIPMENT_NOT_FOUND", 404);
    if (!shipment.labelExternalId)
        throw createShippingError("LABEL_NOT_PURCHASED", 409);
...
async function trackOrderShipment(order) {
    ...
    if (!shipment.trackingCode)
        throw createShippingError("TRACKING_NOT_AVAILABLE", 409);
