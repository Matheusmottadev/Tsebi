...
function mapCalculateResponse(item) {
    const serviceCode = String(item?.id || item?.service || item?.service_id || item?.name || "")
        .trim()
...
function buildSenderAddress() {
    ...
    const responsibleCpf = normalizeCpf(readEnv("SHIP_FROM_RESPONSIBLE_CPF", "") || readEnv("SHIP_FROM_CPF", ""));
    ...
    if (companyDocument && !sender.document) {
        ...
        error.status = 500;
        error.details = {
            message: "Para remetente com CNPJ no Melhor Envio, configure SHIP_FROM_RESPONSIBLE_CPF com um CPF valido.",
...
async function buyLabel({ order }) {
    ...
    const trackingCode = String(orderFromCart?.tracking ||
...
async function track({ trackingCode }) {
    ...
    if (!result) {
        ...
        result = await melhorEnvioRequest(`/me/tracking?tracking_code=${encodeURIComponent(tracking)}`, { method: "GET" });
    }
    return {
...
//# sourceMappingURL=melhorenvio.js.map