...
async function saveShippingQuotes({ orderId = null, userId = null, destinationZip, quotes = [] }) {
    ...
    if (!normalizedZip || !Array.isArray(quotes) || quotes.length === 0)
        return [];
...
async function applyShippingSelectionToOrder({ orderId, provider, serviceName, serviceCode, carrierName, priceCents, deadlineDays, destinationZip }) {
    const normalizedZip = normalizeZip(destinationZip);
    const shippingJsonPatch = {
        ...
        shippingEstimate: deadlineDays == null || Number(deadlineDays) <= 0 ? "" : `${Math.max(0, Number(deadlineDays || 0))} dias`,
        cep: normalizedZip
    };
    const result = await query(`
    UPDATE orders
...
async function upsertShipmentPending({ orderId, provider, serviceCode, priceCents, deadlineDays, rawPayload }) {
    const result = await query(`
...
async function markShipmentLabelPurchased({ orderId, labelExternalId, trackingCode, status = "ETIQUETA_COMPRADA", rawPayload = {} }) {
    const result = await query(`
    UPDATE shipments
    SET
      label_external_id = $2,
...
async function updateShipmentTracking({ orderId, trackingCode, status = "", rawPayload = {} }) {
    const updates = [];
...
//# sourceMappingURL=shipping.queries.js.map