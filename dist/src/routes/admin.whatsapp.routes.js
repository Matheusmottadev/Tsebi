...
const { searchContacts, listVipContacts, upsertVipContact, deleteVipContact, listSendLogs } = require("../../server/lib/whatsapp-repository");
const { estimateTemplateCost, sendNewCollectionToVIP } = require("../../server/lib/whatsapp-service");
...
adminWhatsAppRouter.get("/whatsapp/contacts", async (req, res) => {
    const query = String(req.query.query || "").trim();
    const limit = Number(req.query.limit || 50);
    const rows = await searchContacts({ query, limit });
    return res.json({ rows, count: rows.length });
});
adminWhatsAppRouter.get("/whatsapp/vip", async (req, res) => {
    const query = String(req.query.query || "").trim();
...
adminWhatsAppRouter.post("/whatsapp/vip", async (req, res) => {
...
adminWhatsAppRouter.post("/whatsapp/vip/import", async (req, res) => {
...
adminWhatsAppRouter.delete("/whatsapp/vip/:id", async (req, res) => {
...
adminWhatsAppRouter.get("/whatsapp/vip/estimate", async (req, res) => {
...
adminWhatsAppRouter.post("/whatsapp/vip/send", async (req, res) => {
...
adminWhatsAppRouter.get("/whatsapp/logs", async (req, res) => {
    const limit = Number(req.query.limit || 100);
    const offset = Number(req.query.offset || 0);
...
//# sourceMappingURL=admin.whatsapp.routes.js.map