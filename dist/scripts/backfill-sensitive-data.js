...
async function backfillOrders(dryRun, limit) {
    const result = await query(`
...
async function main() {
    ...
    console.log(`[backfill-sensitive] dry-run=${dryRun ? "1" : "0"} users=${users.updated}/${users.scanned} orders=${orders.updated}/${orders.scanned}`);
}
main().catch((error) => {
    // eslint-disable-next-line no-console
...
//# sourceMappingURL=backfill-sensitive-data.js.map