...
function readAdminAuth(req: any) {
...
async function verifyAndUpgradePassword(user: any, rawPassword: string): Promise<{ ok: boolean; user: any }> {
  ...
  if (!check.needsRehash || !user?.id) return { ok: true, user };
  try {
...
async function getSessionAdminUser(req: any, res: any) {
...
function grantVerifiedStudioSession(req: any, res: any, user: any, currentAuth: any) {
  const now = Date.now();
...
studioAuthRouter.get("/me", async (req: any, res: any) => {
  const auth = readAdminAuth(req);
  if (!auth) {
...
studioAuthRouter.post("/login", adminLoginLimiter, async (req: any, res: any) => {
...
studioAuthRouter.post("/mfa/setup/init", mfaRateLimit, async (req: any, res: any) => {
  const state = await getSessionAdminUser(req, res);
...
studioAuthRouter.post("/mfa/verify", mfaRateLimit, async (req: any, res: any) => {
...
studioAuthRouter.post("/mfa/recovery/regenerate", mfaRateLimit, async (req: any, res: any) => {
  ...
  if (!passwordCheck.ok) return res.status(401).json({ error: "INVALID_CREDENTIALS" });
...
studioAuthRouter.post("/mfa/disable", mfaRateLimit, async (req: any, res: any) => {
  const parsed = mfaSensitiveSchema.safeParse(req.body || {});
  if (!parsed.success) return res.status(400).json({ error: "INVALID_INPUT" });
  if (!isStudioCsrfValid(req)) return res.status(403).json({ error: "CSRF_INVALID" });
