...
function ensureR2Configured() {
  ...
  if (!accountId || !accessKeyId || !secretAccessKey || !bucketName || !publicUrl) {
    const error = new Error("R2_NOT_CONFIGURED") as Error & { code?: string };
...
function normalizeFolder(value: string | undefined, fallback = "tsebi/products"): string {
...
async function uploadBuffer(buffer: Buffer, options: UploadOptions = {}): Promise<UploadResult> {
  ...
  try {
    ...
    const r2Error = new Error(`R2_UPLOAD_FAILED: ${(error as Error).message}`) as Error & {
      code?: string;
