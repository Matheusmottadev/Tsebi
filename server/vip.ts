...
const optionalDigits = (length: any) =>
  z
    .union([z.string(), z.null(), z.undefined()])
    .transform((value: any) => String(value || "").replace(/\D/g, ""))
    .refine((value: any) => value.length === 0 || value.length === length);
...
const vipAdminUpsertSchema = z.object({
  ...
  birthDate: z
    ...
    .refine((value: any) => value.length === 0 || /^\d{4}-\d{2}-\d{2}$/.test(value)),
...
function getAdminTokenFromRequest(req: any) {
...
vipRouter.post("/register", vipRegisterRateLimit, async (req: any, res: any) => {
  try {
...
vipRouter.get("/subscribers", vipListRateLimit, requireVipAdmin, async (req: any, res: any) => {
  try {
    assertVipDbConfigured();
  } catch {
    return res.status(500).json({ error: "VIP_DATABASE_NOT_CONFIGURED" });
...
vipRouter.post("/subscribers", vipListRateLimit, requireVipAdmin, async (req: any, res: any) => {
  try {
    assertVipDbConfigured();
  } catch {
    return res.status(500).json({ error: "VIP_DATABASE_NOT_CONFIGURED" });
...
vipRouter.delete("/subscribers/:id", vipListRateLimit, requireVipAdmin, async (req: any, res: any) => {
  try {
    assertVipDbConfigured();
  } catch {
    return res.status(500).json({ error: "VIP_DATABASE_NOT_CONFIGURED" });
