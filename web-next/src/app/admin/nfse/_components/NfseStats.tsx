...
export function NfseStats({ stats }: NfseStatsProps) {
  const cards = [
    { label: "EMITIDAS NO MES", value: stats.emitidas_mes, sub: null, cor: "#fff" },
    {
    ...
    },
    {
      ...
      cor: stats.pendentes > 0 ? "#8a7a20" : "#fff",
    },
    {
      label: "ERROS",
      value: stats.erros,
      sub: stats.erros > 0 ? "requer atencao" : null,
      cor: stats.erros > 0 ? "#9e3a3a" : "#fff",
    },
  ];
  ...
  return (
    ...
    >
      {cards.map((card) => (
        ...
        >
          <p style={{ fontSize: "10px", letterSpacing: "1px", color: "#444", margin: "0 0 6px" }}>{card.label}</p>
          <p style={{ fontSize: "20px", fontWeight: 500, color: card.cor, margin: 0 }}>{card.value}</p>
          {card.sub ? <p style={{ fontSize: "10px", color: "#555", margin: "3px 0 0" }}>{card.sub}</p> : null}
        </div>
      ))}
    </div>
  );
