window.initProfileSection = function initProfileSection(options = {}) {
  ...
  let serverPreferences = {
    marketing: { email: false, phone: false, sms: false, postal: false },
    contact: { email: false, phone: false, sms: false }
  };
  ...
  function getPreferencesFromUser() {
    ...
    return {
      email: Boolean(serverPreferences.marketing?.email || fallback.email),
      phone: Boolean(serverPreferences.marketing?.phone || fallback.phone),
      sms: Boolean(serverPreferences.marketing?.sms || fallback.sms),
      postal: Boolean(serverPreferences.marketing?.postal || fallback.postal)
    };
  }
  ...
  function renderUserProfile(user) {
    ...
    const contact = {
      email: Boolean(serverPreferences.contact?.email || local.contact?.email),
      phone: Boolean(serverPreferences.contact?.phone || local.contact?.phone),
      sms: Boolean(serverPreferences.contact?.sms || local.contact?.sms)
    };
    if (contactByEmail) contactByEmail.checked = contact.email;
    if (contactByPhone) contactByPhone.checked = contact.phone;
    if (contactBySms) contactBySms.checked = contact.sms;
  ...
  async function saveProfile(data) {
    ...
    const profileResult = await store.updateMyProfile({
      title: data.title,
      name: fullName,
    ...
    let preferencesResult = { ok: true };
    if (store?.updateAccountPreferences) {
      preferencesResult = await store.updateAccountPreferences({
        marketing: data.preferences,
        contact: data.contact
      });
      if (preferencesResult?.ok && preferencesResult.preferences) {
        serverPreferences = preferencesResult.preferences;
      }
    }
  ...
  async function togglePreference(key, value) {
    ...
    if (store?.updateAccountPreferences && !previewMode) {
      const result = await store.updateAccountPreferences({
        marketing: {
          email: Boolean(prefEmail?.checked),
          phone: Boolean(prefPhone?.checked),
          sms: Boolean(prefSms?.checked),
      ...
      if (result?.ok && result.preferences) {
        serverPreferences = result.preferences;
      }
    }
  }
  ...
  function bindEvents() {
    ...
    Object.entries(prefMap).forEach(([id, key]) => {
      ...
      node?.addEventListener("change", () => {
        togglePreference(key, node.checked).catch(() => {});
      });
    });
  ...
  async function loadUserProfile() {
    ...
    if (store?.fetchAccountPreferences) {
      const prefsResult = await store.fetchAccountPreferences();
      if (prefsResult?.ok && prefsResult.preferences) {
        serverPreferences = prefsResult.preferences;
      }
    }
