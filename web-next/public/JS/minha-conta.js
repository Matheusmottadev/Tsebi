...
function renderTrackingOrder(order) {
  ...
  const itemsMarkup = items.length
    ? items
        .map((item) => {
          const image = String(item.image || "images/placeholderreal.webp").trim() || "images/placeholderreal.webp";
          return `
...
function renderUser(user) {
  ...
  if (titleEl) titleEl.textContent = prefix ? `Oḷ, ${prefix} ${firstName}` : `Oḷ, ${firstName}`;
...
function renderFavorites() {
  ...
  favoritesSummary.textContent = uniqueIds.length
    ...
    : "Suas pe̤as salvas para comprar depois.";
  ...
  favoriteItems.forEach((entry) => {
    ...
    const image = String(item?.image || item?.imageUrl || "images/placeholderreal.webp");
