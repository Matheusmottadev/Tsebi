...
function readInternalSecret(req: any) {
...
function getTrackCacheKey(orderNumber: any, email: any) {
  return `${String(orderNumber || "").trim().toUpperCase()}::${String(email || "").trim().toLowerCase()}`;
}

orderTrackingRouter.get("/orders/track", trackRateLimit, async (req: any, res: any) => {
...
orderTrackingRouter.get("/account/orders", requireAuth, async (req: any, res: any) => {
  const orders = await listAccountTrackingOrders(req.session.userId);
  return res.json({ orders });
});

orderTrackingRouter.post("/internal/jobs/melhorenvio-sync", async (req: any, res: any) => {
