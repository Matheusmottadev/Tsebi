...
async function loadOrderOr404(orderId: any) {
...
adminShippingRouter.post("/orders/:id/shipping/buy-label", async (req: any, res: any) => {
...
adminShippingRouter.get("/orders/:id/shipping/label", async (req: any, res: any) => {
  const parsedParams = orderIdSchema.safeParse(req.params || {});
  if (!parsedParams.success) {
    return res.status(400).json({ ok: false, error: "INVALID_ID" });
  }
...
adminShippingRouter.get("/orders/:id/shipping/track", async (req: any, res: any) => {
  const parsedParams = orderIdSchema.safeParse(req.params || {});
  if (!parsedParams.success) {
    return res.status(400).json({ ok: false, error: "INVALID_ID" });
  }
...
adminShippingRouter.post("/orders/:id/shipping", async (req: any, res: any) => {
  const parsedParams = orderIdSchema.safeParse(req.params || {});
