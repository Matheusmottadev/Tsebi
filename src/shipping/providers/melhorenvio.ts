...
function readBooleanEnv(name: any, fallback: any) {
...
function mapCalculateResponse(item: any) {
  const serviceCode =
...
function buildQuoteProducts(packages: any = []) {
  ...
  return list.map((entry: any, index: any) => ({
    id: `pkg-${index + 1}`,
...
async function melhorEnvioRequest(path: any, { method = "GET", body }: any = {}) {
...
async function quote({ fromZip, toZip, packages = [] }: any) {
  ...
  const normalized = list
    ...
    .filter((entry: any) => entry.priceCents >= 0 && entry.serviceCode && entry.serviceName);
...
function buildProductsForLabel(order: any) {
  const fallbackWeight = Math.max(0.1, toNumber(process.env.DEFAULT_PACKAGE_WEIGHT_KG || 0.3, 0.3));
  const fallbackLength = Math.max(1, Math.round(toNumber(process.env.DEFAULT_PACKAGE_LENGTH_CM || 20, 20)));
  const fallbackWidth = Math.max(1, Math.round(toNumber(process.env.DEFAULT_PACKAGE_WIDTH_CM || 15, 15)));
  const fallbackHeight = Math.max(1, Math.round(toNumber(process.env.DEFAULT_PACKAGE_HEIGHT_CM || 5, 5)));
...
function normalizeCnpj(value: any) {
  const digits = String(value || "").replace(/\D/g, "");
...
function toAddressLine(street: any, number: any) {
...
function rebalanceProductsInsurance(products: any, cap: any) {
  const list = Array.isArray(products) ? products : [];
  if (!list.length) return [];
  const safeCap = Math.max(1, toMoney(cap));

  const total = list.reduce((sum: any, item: any) => {
...
function capProductsUnitaryValue(products: any, cap: any) {
  const list = Array.isArray(products) ? products : [];
  const safeCap = Math.max(1, toMoney(cap));
  return list.map((item: any) => ({
    ...item,
...
function uniqueServiceCandidates(primaryServiceCode: any, quoteList: any = []) {
  const seen = new Set();
  ...
  (Array.isArray(quoteList) ? quoteList : []).forEach((quoteEntry: any) => {
...
async function buyLabel({ order }: any) {
  ...
  const productsTotal = rawProducts.reduce((sum: any, product: any) => {
...
async function getLabel({ labelExternalId }: any) {
...
async function track({ trackingCode }: any) {
  ...
  try {
    ...
    if (batch && typeof batch === "object") {
      ...
      result =
        list.find((entry: any) => String(entry?.tracking || "").trim().toUpperCase() === tracking.toUpperCase()) ||
        list.find((entry: any) => String(entry?.melhorenvio_tracking || "").trim().toUpperCase() === tracking.toUpperCase()) ||
        list[0] ||
        null;
    }
  } catch {
