...
function buildDummyPriceByZip(zip: any) {
...
function packageMultiplier(packages: any = []) {
  const totalQuantity = Array.isArray(packages)
    ? packages.reduce((sum: any, item: any) => sum + Math.max(1, Number(item?.quantity || 1)), 0)
    : 1;
  return Math.max(1, totalQuantity);
}

async function quote({ toZip, packages = [] }: any) {
