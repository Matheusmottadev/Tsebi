...
function normalizeOrderLookup(value: any) {
  return String(value || "")
    .replace(/[^a-zA-Z0-9]/g, "")
    .toUpperCase();
}
...
function mapTrackingEventRow(row: any) {
  if (!row) return null;
  return {
    id: row.id,
...
function mapTrackableOrderRow(row: any, items: any = []) {
  if (!row) return null;
...
async function listOrderItems(orderId: any) {
...
async function findTrackableOrderByOrderNumberAndEmail(orderNumber: any, email: any) {
...
async function listTrackableOrdersByUserId(userId: any, limit: any = 100) {
  if (!userId) return [];
  ...
  const orders = result.rows.map((row: any) => mapTrackableOrderRow(row, []));
...
async function listTrackingEventsByOrderId(orderId: any, direction: any = "ASC") {
...
async function insertTrackingEvents(orderId: any, events: any = []) {
...
async function updateOrderTrackingState(orderId: any, patch: any = {}) {
  const updates: any[] = [];
  const values = [orderId];

  const push = (column: any, value: any, cast: any = "") => {
    values.push(value);
...
async function listTrackableOrdersForSync(limit: any = 120) {
  ...
  return result.rows.map((row: any) => mapTrackableOrderRow(row, []));
}
