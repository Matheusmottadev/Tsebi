...
function normalizeZip(value: any) {
  return String(value || "").replace(/\D/g, "").slice(0, 8);
}

function mapShippingQuoteRow(row: any) {
  if (!row) return null;
  return {
    id: row.id,
...
function mapShipmentRow(row: any) {
  if (!row) return null;
  return {
    id: row.id,
...
async function saveShippingQuotes({ orderId = null, userId = null, destinationZip, quotes = [] }: any) {
...
async function findShippingQuoteById(quoteId: any) {
...
async function assignShippingQuoteToOrder(quoteId: any, orderId: any) {
  const result = await query(
    `
...
async function findShipmentByOrderId(orderId: any) {
  const result = await query(
    `
    SELECT *
    FROM shipments
...
async function listShipmentsByOrderIds(orderIds: any = []) {
