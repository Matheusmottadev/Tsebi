(function initAccountOrdersModule() {
  ...
  window.initOrdersSection = function initOrdersSection(context) {
    ...
    function detailHtml(order) {
      ...
      const addressText = shipping && typeof shipping === 'object'
      ...
      const trackingBlock = order.trackingUrl
        ...
        : order.trackingCode
          ? `<p class="conta-muted">Código de rastreio: <strong>${escapeHtml(order.trackingCode)}</strong></p>`
          : '';
      ...
      if (!orders.length) {
        listMount.innerHTML = `
          <div class="orders-empty">
            ...
            <a class="btn-primary" href="index.html">Começar a comprar</a>
          </div>
        `;
