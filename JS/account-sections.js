(function initAccountExtraSections() {
  ...
  window.initPrivateCareSection = function initPrivateCareSection(context) {
    ...
    function getHistory() {
      const list = Array.isArray(root[userId]) ? root[userId] : [];
    ...
    [prefEmail, prefPhone, prefSms].forEach((el) => el && el.addEventListener('change', savePrefs));
  ...
  window.initWishlistSection = function initWishlistSection(context) {
    ...
    function renderWishlist() {
      ...
      if (!items.length) {
        grid.innerHTML = '<div class="orders-empty"><p>Sua Lista de Desejos está vazia.</p><a class="btn-primary" href="index.html">Explorar peças</a></div>';
        return;
      }
